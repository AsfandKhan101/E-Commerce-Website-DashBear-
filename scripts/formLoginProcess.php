...
Process the Login form data and set up the necessary session data for
the user to shop. If incorrect Login information (bad username and/or
bad password) is entered, the user is redirected back to the login form.
*/
session_start();
...
if ($numRecords == 0)
{   //No records retreived ; username entered not in database; go back to form 
...
if ($numRecords == 1)
{
    ...
    if(md5($_POST['loginPassword']) == $row['login_password'])
    {
        ...
        $_SESSION['middle_intial'] = $row['middle_initial'];
